...
6. **Vision-Derived Naming & No Prefixes:** Component labels and screen names from the workspace are rough suggestions only (shown as `[TODO: ... Suggestion: X]` in the scaffold). Always derive accurate, descriptive Vietnamese labels from the visual content of the cropped images. Do NOT include generic prefixes such as "Màn hình", "Trang", "Screen", "Page", "Component", or "Thành phần" in `screen.name` or `component.label` (e.g. write "Cài đặt" instead of "Màn hình Cài đặt" or "Trang Cài đặt"). The document compiler automatically prepends the correct prefix ("Màn hình " or "Component ") during generation.
...
10. **Strict "Fill in the Blank" Operation:** The AI must solely "fill in the blank" created by the pre-analysis step. For the visual UI structure (`components` and `children`), you must solely fill in the `[TODO: ...]` blanks without altering the arrays. You are strictly forbidden from modifying the structural arrays (adding or removing components or children). However, you ARE expected to append items to the empty `interactions[]` and `apis[]` arrays based on your source code analysis.
...
1. **Screen name and description** — replace the suggestion placeholders in `screen.name` and `screen.description` with real Vietnamese values derived from the full screenshot and corresponding screen code. Write `screen.name` without any generic prefixes (e.g. write "Cài đặt" instead of "Màn hình Cài đặt"). Write `screen.description` as a true high-level summary (mô tả chung) of the screen's role/purpose, NOT a list or restatement of its UI elements (since we have the UI elements/children table for that).
2. **Component labels and descriptions** — replace the label suggestion placeholders in the `components[]` array with accurate, descriptive Vietnamese labels without any generic prefixes (e.g. write "Header" instead of "Component Header"). Write the component description as a true high-level summary (mô tả chung) of its role/purpose, NOT simply re-stating the UI elements or children inside it (since we have the UI elements/children table for that).
...
4. **`children[]`** for each non-leaf component — the scaffold automatically populates nested child components via `componentId`. **Do NOT manually append unannotated UI elements to this array.** To prevent confusion and ensure a 1-to-1 match between the data tables and the image boxes, you must only document elements that were explicitly boxed in the GUI. Simply ignore any unannotated elements and proceed normally.
