...
### Step 1: Annotate
1. Call `launch_annotator(path=...)` if you have a local image or a previously exported workspace to start with, or just `launch_annotator()` if the user will manually import one.
...
2. Export cropped component image assets: call `export_images(output_path="./workspace_xyz")`. This extracts both clean crops (for AI vision analysis) and annotated crops with bounding box/ID overlays (for document compilation) into the target directory.

### Step 3: Analyze
1. **Vision analysis:** Inspect the screenshot and raw component images (the clean crops without bounding box overlays) from the `./workspace_xyz` directory. Categorize components using the rules in `tlgp://spec/classification-guide`. Fill in component details, child control types, descriptions, and interactions.
