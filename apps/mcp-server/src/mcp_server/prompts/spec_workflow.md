...
7. **Unit Limit:** Each component and the screen have a strict, non-configurable complexity budget of 15 units (1 unit per annotation, 3 per API). Validation will reject any scope that exceeds this limit. You MUST NOT attempt to resolve this yourself (e.g. by restructuring the analysis, removing children, moving APIs, or editing any configuration). Instead, stop and ask the user to re-annotate the image in the Annotation Tool — for example, by grouping child elements under a new parent component to reduce the count at the offending level. After the user re-annotates, re-run `prepare_analysis` to regenerate the scaffold.
