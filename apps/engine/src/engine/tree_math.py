...
from models import WorkspaceState, Component
...
def recalculate_tree(state: WorkspaceState):
    ...
    def process_node(comp_id: UUID, parent_absolute_x: int, parent_absolute_y: int, number_prefix: str):
        ...
        sorted_children = sorted(
            valid_children, 
            key=lambda cid: state.components[cid].bounds.y
        )
    ...
    sorted_roots = sorted(
        valid_roots,
        key=lambda rid: state.components[rid].bounds.y
    )
