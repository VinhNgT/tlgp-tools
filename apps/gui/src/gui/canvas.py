...
class AnnotationCanvas(tk.Canvas):
    ...
    def __init__(self, parent, client: EngineClient):
        ...
        self.drag_data = {"item_id": None, "start_x": 0, "start_y": 0, "comp_id": None, "mode": None}
    ...
    def render_state(self, state: WorkspaceState):
        ...
        for comp_id, comp in state.components.items():
            ...
            self.create_text(
                b.left + 15, b.top + 10,
                text=comp.number, fill=outline_color, font=("Arial", 10, "bold")
            )
    ...
    def on_press(self, event):
        ...
        if not items:
            ...
            self.draw_rect_id = self.create_rectangle(cx, cy, cx, cy, outline="green", width=2)
            # Re-render to clear selection highlight instantly if needed
            if self.client.state: self.render_state(self.client.state)
            return
    ...
    def on_drag(self, event):
        ...
        if mode == "draw" and self.draw_rect_id:
            self.coords(self.draw_rect_id, self.drag_data["start_x"], self.drag_data["start_y"], cx, cy)
    ...
    def on_release(self, event):
        ...
        if mode == "draw" and self.draw_rect_id:
            ...
            if right - left > 10 and bottom - top > 10:
                ...
                if lbl:
                    bounds = {"left": int(left), "top": int(top), "right": int(right), "bottom": int(bottom)}
                    try:
            ...
            try:
                self.client.move_component(self.drag_data["comp_id"], int(coords[0]), int(coords[1]))
            except Exception as e:
                print(f"Failed to move: {e}")
                
        elif mode == "resize_br" and self.drag_data["item_id"]:
            coords = self.coords(self.drag_data["item_id"])
            bounds = {"left": int(coords[0]), "top": int(coords[1]), "right": int(coords[2]), "bottom": int(coords[3])}
            try:
