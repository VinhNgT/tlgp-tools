...
class ScreenInfoDialog(tb.Toplevel):
    def __init__(self, parent, screen_name="", description=""):
        ...
        tb.Label(frame, text="Screen Name (e.g. Product Details Screen):", anchor="w").pack(fill=tk.X, pady=(0, 5))
        self.entry_name = tb.Entry(frame, width=50)
        self.entry_name.insert(0, screen_name)
        self.entry_name.pack(fill=tk.X, pady=(0, 10))
        self.entry_name.focus_set()

        tb.Label(frame, text="Functional Description:", anchor="w").pack(fill=tk.X, pady=(0, 5))
        ...
        btn_cancel = tb.Button(btn_frame, text="Cancel", command=self.on_cancel, width=10, bootstyle="secondary")
        btn_cancel.pack(side=tk.RIGHT, padx=(5, 0))
        
        btn_save = tb.Button(btn_frame, text="Save", command=self.on_save, width=10, bootstyle="primary")
        btn_save.pack(side=tk.RIGHT)

        # Bind Enter and Escape keys. Do not trigger save if a multiline text area is focused.
        self.bind("<Return>", lambda e: self.on_save() if self.focus_get() != self.text_desc else None)
    ...
    def on_save(self, event=None):
        ...
        if not name:
            messagebox.showwarning("Warning", "Please enter a screen name!", parent=self)
            return
...
class EditLabelDialog(tb.Toplevel):
    def __init__(self, parent, title="Edit Component Name", prompt="Enter Name:", current_value=""):
        super().__init__(parent)
        self.title(title)
        ...
        btn_cancel = tb.Button(btn_frame, text="Cancel", command=self.on_cancel, width=8, bootstyle="secondary")
        btn_cancel.pack(side=tk.RIGHT, padx=(5, 0))
        
        btn_ok = tb.Button(btn_frame, text="OK", command=self.on_save, width=8, bootstyle="primary")
