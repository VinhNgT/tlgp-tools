...
from tlgp_doc_generator.cli import main


class TestCliHelp:
    def test_no_args_prints_help(self, capsys):
        with patch("sys.argv", ["tlgp-doc-gen"]):
    ...
    def test_help_flag(self, capsys):
        with patch("sys.argv", ["tlgp-doc-gen", "--help"]):
            with pytest.raises(SystemExit) as exc:
                main()
            assert exc.value.code == 0
...
class TestCliValidation:
    def test_nonexistent_file(self, capsys):
        with patch("sys.argv", ["tlgp-doc-gen", "/nonexistent/analysis.json"]):
            with pytest.raises(SystemExit) as exc:
                main()
    ...
    def test_invalid_json(self, tmp_path, capsys):
        ...
        with patch("sys.argv", ["tlgp-doc-gen", str(bad_json)]):
            with pytest.raises(SystemExit) as exc:
                main()
            assert exc.value.code == 1
        captured = capsys.readouterr()
    ...
    def test_invalid_schema(self, tmp_path, capsys):
        ...
        with patch("sys.argv", ["tlgp-doc-gen", str(bad_schema)]):
            with pytest.raises(SystemExit) as exc:
                main()
            assert exc.value.code == 1
        captured = capsys.readouterr()
...
class TestCliDryRun:
    def test_dry_run_prints_summary(self, tmp_path, capsys):
        ...
        with patch("sys.argv", ["tlgp-doc-gen", str(json_path), "--dry-run"]):
            main()
...
class TestCliGeneration:
    def test_generates_docx(self, tmp_path):
        ...
        with patch("sys.argv", ["tlgp-doc-gen", str(json_path), "-o", str(output_path)]):
            main()
    ...
    def test_default_output_name(self, tmp_path):
        ...
        with patch("sys.argv", ["tlgp-doc-gen", str(json_path)]):
            main()
